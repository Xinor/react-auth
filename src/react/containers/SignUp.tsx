...
import { effectSignUp } from "../../redux/effects";
...
export default function SignUp() {
    ...
    const [form, setForm] = useState<ISignUp>({ email: '', password: '' });
    ...
    const submitHandler: React.FormEventHandler = async (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        dispatch(effectSignUp(form));
    }

    const goToSignIn: React.MouseEventHandler = () => {
    ...
    return (
        <div className='flex-1 flex justify-center items-center m-40'>
            <form className="inline-flex flex-col bg-white shadow-sm rounded px-8 pt-6 pb-8 mb-4">
                ...
                <Button title='Sign In?' onClick={goToSignIn} className='mt-4 w-28 mx-auto' />
            </form>
        </div>
    )
}